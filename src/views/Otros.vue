...
export default {
  ...
  methods:{
    getStartAnimes: async function(){
      await firebase.firestore().collection(this.$route.params.collection).orderBy('actualizado_en', 'desc').where('email', '==', firebase.auth().currentUser.email).limit(6).onSnapshot((snapshot) => {
        this.animes = [];
    ...
    addAnimes: async function(){
      await firebase.firestore().collection(this.$route.params.collection).orderBy('actualizado_en','desc').where('email', '==', firebase.auth().currentUser.email).startAfter(this.animes[this.animes.length-1].actualizado_en).limit(6).onSnapshot((snapshot) => {
        snapshot.forEach((doc) => { 
          let data = doc.data();
          Object.assign(data, {id: doc.id});
          Object.assign(data, {col: this.$route.params.collection})
