<template>
  <div class="d-inline-block">
    <v-lazy transition="
    fade-transition">
      <v-card max-width="225px" class="ma-2">
        ...
        <v-card-actions @mouseleave="saveChapter">
          <v-btn class="mr-0" v-if="showChapter && data.capitulo > 0" @click="data.capitulo--" icon>
            <v-icon>mdi-arrow-left-drop-circle</v-icon>
          </v-btn>
          <v-btn class="ml-0" v-if="showChapter" @click="data.capitulo++" icon>
            <v-icon>mdi-arrow-right-drop-circle</v-icon>
          </v-btn>
          <v-spacer />
          <v-btn class="mr-0" icon @click="edit">
            <v-icon>mdi-pencil</v-icon>
          </v-btn>
          <v-btn class="ml-0" icon @click.stop="dialog = true">
          ...
          <v-menu offset-y>
            ...
            <v-list>
              <v-list-item @click="move('viendo')">Mover a viendo</v-list-item>
              <v-list-item @click="move('vistos')">Mover a vistos</v-list-item>
              <v-list-item @click="move('favoritos')">Mover a favoritos</v-list-item>
              <v-list-item @click="move('abandonados')">Mover a abandonados</v-list-item>
              <v-list-item @click="move('pendientes')">Mover a pendientes</v-list-item>
            </v-list>
          </v-menu>
        </v-card-actions>
      </v-card>
    ...
    <v-dialog v-model="dialog" max-width="290">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="green darken-1" text @click="remove">Si</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
